...
def demo(cfg):
    ...
    if cfg.TEST_DATA_PATH is not None and cfg.SEQ_LIST is None:
    ...
    elif cfg.SEQ_LIST is not None:
        sequences = [seq.strip() for seq in cfg.SEQ_LIST]
        print(sequences)
        image_root = os.path.join(cfg.TEST_DATA_PATH, 'JPEGImages_10fps')
        label_root = os.path.join(cfg.TEST_DATA_PATH, 'Annotations')
    else:
        image_root = cfg.TEST_FRAME_PATH
        label_root = cfg.TEST_LABEL_PATH
        sequences = [image_root.split('/')[-2]]
    ...
    for seq_name in sequences:
        ...
        if cfg.TEST_DATA_PATH is not None:
            ...
            seq_images = np.sort(os.listdir(image_root))
            image_root = "/".join(image_root.split('/')[:-2])
            label_root = "/".join(label_root.split('/')[:-2])
            ...
            for frame_idx, samples in enumerate(seq_dataloader):
                ...
                if frame_idx == 0:
                    ...
                    pred_logit = engine.match_propogate_one_frame(
                        current_img, output_size=(output_height, output_width))
...
#CUDA_VISIBLE_DEVICES=0 python tools/demo.py --seq_list datasets/VOST/ImageSets/val.txt --ckpt_path pretrain_models/vost_aot_best.pth --output_path demo_output/aot_best_10fps
def main():
    import argparse
    parser = argparse.ArgumentParser(description="AOT Demo")
    parser.add_argument('--exp_name', type=str, default='default')

    parser.add_argument('--stage', type=str, default='pre_vost')
    ...
    parser.add_argument('--ckpt_path',
                        type=str,
                        default='./pretrain_models/aotplus_R50_AOTL_Temp_pe_Slot_4_ema_20000.pth')
    ...
    if len(args.seq_list) != 0:
        lst_file = open(args.seq_list, 'r')
        lst = lst_file.readlines()
        cfg.SEQ_LIST = lst
        cfg.TEST_DATA_PATH = '/mnt/fsx/VOST/'
        cfg.TEST_FRAME_PATH = None
    elif len(args.seq_name) != 0:
        cfg.TEST_DATA_PATH = None
        cfg.TEST_SEQ_LIST = None
        cfg.SEQ_LIST = None
        cfg.TEST_FRAME_PATH = 'datasets/VOST/JPEGImages_10fps/%s/' % args.seq_name
        cfg.TEST_LABEL_PATH = 'datasets/VOST/Annotations/%s/' % args.seq_name
    else:
