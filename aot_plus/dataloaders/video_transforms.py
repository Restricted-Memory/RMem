...
class BalancedRandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        while (not is_contain_obj) and (step < self.max_step):
            ...
            if all_obj[-1] == 0 or (len(all_obj) == 1 and all_obj[-1] == 255) or (len(all_obj) > 1 and all_obj[-2] == 0 and all_obj[-1] == 255):
                continue
        ...
        for elem in after_crop:
            ...
            for idx in range(len(all_obj)):
                ...
                if obj_id != 255:
                    ...
                    new_elem += (mask * 255).astype(np.uint8)
